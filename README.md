# Sistema-controle-de-contas
Primeiro projeto de portfólio desenvolvido para praticar persistência de dados em Python utilizando JSON.

Sistema de Controle de Contas Mensais

Projeto desenvolvido em Python para controle de contas mensais, com persistência de dados em JSON.

Funcionalidades atuais

Cadastro de contas

Validação de dia e mês de vencimento

Controle de contas fixas

Cálculo do total a pagar

Armazenamento em arquivo contas.json

Leitura automática das contas já salvas

Listagem formatada das contas

Tecnologias utilizadas

Python 3

Módulo json

Como executar
python main.py
Estrutura do projeto
controle-contas/
│
├── main.py
├── contas.json
└── README.md
Próximas melhorias

Remover contas cadastradas

Marcar contas como pagas

Interface gráfica com Tkinter

Relatórios mensais

Objetivo do projeto

Este projeto faz parte do meu portfólio de estudos em Python, com foco em lógica de programação, manipulação de arquivos, listas, dicionários e persistência de dados.
